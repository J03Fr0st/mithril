...
#
# Sync this mithril checkout → J03Fr0st/openai-codex-plugins.
...
# plugins/mithril/ when absent, then copies the tracked plugin files from
...
FORK="J03Fr0st/openai-codex-plugins"
DEFAULT_BASE="main"
DEST_REL="plugins/mithril"
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  SYNC_BRANCH="bootstrap/mithril-${UPSTREAM_SHORT}-${TIMESTAMP}"
else
  SYNC_BRANCH="sync/mithril-${UPSTREAM_SHORT}-${TIMESTAMP}"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  echo "Mode:     BOOTSTRAP (creating plugins/mithril/ when absent)"
fi
echo ""
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  COMMIT_TITLE="bootstrap mithril v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Initial bootstrap of the mithril plugin from upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).

Creates \`plugins/mithril/\` by copying the tracked plugin files from upstream, including \`.codex-plugin/plugin.json\`, \`assets/\`, and \`hooks/\`.

Run via: \`scripts/sync-to-codex-plugin.sh --bootstrap\`
Upstream commit: https://github.com/J03Fr0st/mithril/commit/$UPSTREAM_SHA
...
This is a one-time bootstrap. Subsequent syncs will be normal (non-bootstrap) runs using the same tracked upstream plugin files."
  ...
  COMMIT_TITLE="sync mithril v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="automated sync from Mithril upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).
...
Upstream: https://github.com/J03Fr0st/mithril/commit/$UPSTREAM_SHA
Branch:   $SYNC_BRANCH"
