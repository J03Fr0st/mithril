...
const MITHRIL_VERSION = readMithrilVersion();
const MITHRIL_BRAND_IMAGE_URL = 'https://primeradiant.com/brand/mithril-visual-brainstorming-logo.png';
const TELEMETRY_DISABLE_ENV_VARS = [
  'MITHRIL_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const MITHRIL_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(MITHRIL_VERSION);
  const text = MITHRIL_TELEMETRY_DISABLED
    ? 'Prime Radiant Mithril v' + version
    : 'Mithril v' + version;
  const logo = MITHRIL_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + MITHRIL_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(MITHRIL_VERSION) + '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">';

  return '<div class="brand"><a href="https://github.com/obra/mithril">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
